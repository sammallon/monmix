...
- **M7**: display power management — connectivity-driven. While the system can drive a mix (WiFi up + MS WS connected + physical console attached), the panel stays awake indefinitely; there is no user-selectable sleep timeout. Any degraded state caps the awake budget at 60 s from last touch (a 30-s WARN countdown that any tap dismisses, then sleep). Sleep blanks the panel and gracefully releases the MS connection (unsubscribe + WS close + worker join) so no broadcasts go to a dark panel. Wake mechanisms: tap the blanked panel, or wait for an MS-reconnect / console-reattach edge (the device auto-wakes when the rig comes back). A top-bar power-icon button forces immediate sleep on demand. Auto-wake on MS reconnect arms only when the device was degraded at sleep entry or mid-sleep, so a manual sleep with a healthy connection doesn't spuriously wake. The device probes MS every 30 s while asleep to detect recovery.
