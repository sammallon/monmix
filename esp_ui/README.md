...
- **M4** (done 2026-05-04): 3D-printed mic-stand enclosure. Print run completed; STLs in `hardware/`.
...
- **M8** (in flight as part of UI polish batch 2026-05-04): display preferences. Settings panel gains a backlight brightness slider (LEDC PWM, sensible floor so a mis-tap can't render the panel unreadable) and a 180° rotation toggle (for mic-stand mounting orientation flexibility — 90/270 not offered, the fader UI is landscape-only). Both persisted to NVS (with SD-mirror) and applied at boot.
