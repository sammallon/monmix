...
def test_esp_ui(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_esp_ui_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@pytest.mark.macos
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_esp_ui_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_esp_ui_host(app: QemuApp, dut: QemuDut) -> None:
