...
// most recently used. Loaded on init, rewritten on every mutation.
typedef struct {
...
// Saved-networks blob I/O. Helpers stay local since the format isn't
...
static void load_saved_networks(void)
{
    ...
    // Defensive NUL-termination so a truncated/corrupt blob can't leak past
    // the field bounds when callers strncpy from these.
...
bool app_config_wifi_saved_add(const char *ssid, const char *pass)
{
    ...
    if (existing >= 0) {
        // Promote: shift [0..existing-1] down by one, then write at index 0.
        // This keeps the most-recently-used invariant under both first-add
        // and re-save-with-edited-password.
